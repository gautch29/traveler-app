...
struct StaySummaryCardView: View {
    @Binding var step: Step
    let isTimelineEditMode: Bool
    let store: TripStore
    @Binding var selectedFileToView: IdentifiableURL?
    @Binding var fileViewTitle: String
    @Binding var showingFilePicker: Bool
    @Binding var fileImportCallback: ((URL) -> Void)?
    let deleteAction: () -> Void
    
    var body: some View {
        guard let stay = step.stayInfo else { return AnyView(EmptyView()) }
                ...
                if isTimelineEditMode {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("City Name", text: Binding(
                            get: { step.stayInfo?.cityName ?? "" },
                            set: { step.stayInfo?.cityName = $0 }
                        ))
                        .font(.subheadline)
                        .glassTextFieldStyle()
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if isTimelineEditMode {
                            TextField("Hotel Title", text: Binding(
                                get: { step.stayInfo?.hotel?.title ?? "" },
                                set: { step.stayInfo?.hotel?.title = $0 }
                            ))
                            .font(.headline)
                            .glassTextFieldStyle()
                            
                            TextField("Hotel Details", text: Binding(
                                get: { step.stayInfo?.hotel?.details ?? "" },
                                set: { step.stayInfo?.hotel?.details = $0 }
                            ))
                            .font(.subheadline)
                            .glassTextFieldStyle()
                        } else {
                                ...
                                Button {
                                    fileImportCallback = { url in
                                        if let tempURL = stageFile(selectedURL: url, type: "ticket") {
                                            if var hotel = step.stayInfo?.hotel {
                                                hotel.sharedFiles.append(tempURL)
                                                step.stayInfo?.hotel = hotel
                                            } else {
                                                step.stayInfo?.hotel = TripItem(
                        ...
                        if !isTimelineEditMode {
                            ...
                            let hotelPasses = stay.hotel?.getWalletPasses(forUser: user) ?? []
...
struct StayDaysSectionView: View {
    @Binding var step: Step
    let isTimelineEditMode: Bool
    let store: TripStore
    @Binding var editingDayIds: Set<String>
    ...
    private func dayBinding(dayId: String) -> Binding<DayInfo> {
        return Binding(
            get: {
                step.stayInfo?.days.first(where: { $0.id == dayId }) ?? DayInfo(dayNumber: 0, date: "", title: "", description: "", items: [])
            },
            set: { newValue in
                guard var stay = step.stayInfo else { return }
    ...
    private func itemBinding(dayId: String, itemId: String) -> Binding<TripItem> {
        let dayBind = dayBinding(dayId: dayId)
        return Binding(
            get: {
                dayBind.wrappedValue.items.first(where: { $0.id == itemId }) ?? TripItem(id: "", type: .activity, title: "", time: "", details: "", sharedFiles: [], profileFiles: nil, walletPasses: nil, profileWalletPasses: nil, websiteURL: nil, flightNumber: nil, mapPlace: nil)
            },
            set: { newValue in
                var day = dayBind.wrappedValue
                if let idx = day.items.firstIndex(where: { $0.id == itemId }) {
                    day.items[idx] = newValue
                    dayBind.wrappedValue = day
                }
            }
        )
    }
    ...
    var body: some View {
        ...
        return AnyView(
            VStack(alignment: .leading, spacing: 16) {
                if isTimelineEditMode {
                    Button {
                        let newDayNumber = (step.stayInfo?.days.count ?? 0) + 1
                        let newDay = DayInfo(
                        ...
                        withAnimation {
                            step.stayInfo?.days.append(newDay)
                        }
                    } label: {
                        HStack {
                ...
                ForEach(stay.days) { day in
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if isDayEditing {
                            let dayBind = dayBinding(dayId: day.id)
                            VStack(alignment: .leading, spacing: 8) {
                                HStack {
                            ...
                            HStack {
                                ...
                                if isTimelineEditMode {
                                    Button(role: .destructive) {
                                        if let idx = step.stayInfo?.days.firstIndex(where: { $0.id == day.id }) {
                                            withAnimation {
                                                _ = step.stayInfo?.days.remove(at: idx)
                                            }
                                        }
                                    } label: {
                            ...
                            .onTapGesture {
                                withAnimation(.spring(response: 0.35, dampingFraction: 0.8)) {
                                    if expandedDays.contains(day.id) {
                                        _ = expandedDays.remove(day.id)
                                    } else {
                                        _ = expandedDays.insert(day.id)
                                    }
                                }
                            }
                        }
                                ...
                                if !isDayEditing && day.items.isEmpty {
                                    ...
                                    VStack(spacing: 12) {
                                        ForEach(day.items) { item in
                                            if isDayEditing {
                                                let itemBind = itemBinding(dayId: day.id, itemId: item.id)
                                                VStack(alignment: .leading, spacing: 6) {
                                                    HStack {
                                                        ...
                                                        Button(role: .destructive) {
                                                            let dayBind = dayBinding(dayId: day.id)
                                                            if let idx = dayBind.wrappedValue.items.firstIndex(where: { $0.id == item.id }) {
                                                                withAnimation {
                                                                    _ = dayBind.wrappedValue.items.remove(at: idx)
                                                                }
                                                            }
                                                        } label: {
                                                            Image(systemName: "minus.circle.fill")
